...
class AttendancePDFExtractor:
    ...
    def parse_date_headers(self, header_rows):
        """Parse date information from header rows"""
        dates = []
        # Typically dates are in format like "Wed 01", "Thu 02", etc.
        if len(header_rows) >= 2:
            days = header_rows[0]  # Day names
            date_nums = header_rows[1]  # Date numbers

            for i in range(len(days)):
                if days[i] and date_nums[i]:
                    # Clean and combine day and date
                    day = str(days[i]).strip()
                    date_num = str(date_nums[i]).strip()
                    if day and date_num and date_num.isdigit():
                        dates.append(f"{day}-{date_num}")
    ...
    def parse_attendance_data(self):
        ...
        for table in self.raw_tables:
            ...
            # Find header rows (usually first 2-3 rows contain date info)
            header_start = 0
            data_start = None

            # Find where actual employee data starts
            for i, row in enumerate(table):
                if row and len(row) > 0:
                    first_cell = str(row[0]).strip().upper()
                    # Look for employee names or skip headers
                    if first_cell and first_cell not in ['EMPLOYEE', '', 'NONE']:
                        data_start = i
                        break
            ...
            # Extract date headers (assume first few rows before data)
            header_rows = table[max(0, data_start-2):data_start]
            ...
            for row in table[data_start:]:
                ...
                # Clean names and remove newlines
                employee_name = str(row[0]).strip().replace('\n', ' ').replace('\r', ' ') if row[0] else ""
                # Remove extra spaces
                employee_name = ' '.join(employee_name.split())
    ...
    def create_dataframe(self):
        ...
        # Check if last column is Paid_time (contains time format like "190:00")
        # Sample last column from first few records
        is_paid_time_col = False
        if data:
            sample_last_values = [rec['Attendance_Codes'][-1] for rec in data[:5] if rec['Attendance_Codes']]
            # Check if values match time pattern (digits + ":" + digits)
            time_pattern_count = sum(1 for val in sample_last_values if val and ':' in val and
                                     any(c.isdigit() for c in val.split(':')[0]))
            if time_pattern_count >= 3:  # If 3+ samples look like time
                is_paid_time_col = True
                max_days -= 1  # Exclude last column from day count
    ...
    def get_month_from_filename(self):
        """Extract month/year from filename"""
        filename = Path(self.pdf_path).stem
        # Assuming format like "OCT_2025_SO-SC"
        match = re.search(r'([A-Z]+)_(\d{4})', filename)
        if match:
            month = match.group(1)
            year = match.group(2)
...
def extract_all_pdfs(folder_path):
    ...
    for pdf_file in pdf_files:
        ...
        extractor = AttendancePDFExtractor(str(pdf_file))
        df = extractor.create_dataframe()

        if not df.empty:
            month_year = extractor.get_month_from_filename()
            all_dataframes[month_year] = df
